...
class PhonePipeline(object):
    ...
    def _conditional_insert(self,tx,item):
        tx.execute("select * from phone87 where m_name= %s",(item['name'][0]))
        result=tx.fetchone()
        ...
        if result:
            ...
            tx.execute(\
                "insert into phone77 (m_name,re_price,m_score,m_battery,m_screen,m_design,m_fun,m_run,m_cp,m_category,m_image) values (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",\
